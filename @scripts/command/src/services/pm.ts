...
export const select = async (message = '请选择模块', multiple = false, yes = false, lernaPackages: string | string[] = [], lernaIgnorePackages: string | string[] = []) => {
  ...
  if (!(Array.isArray(lernaPackages) && lernaPackages.length > 0)) {
    const json = await fs.readJSON(path.join(basePath, 'lerna.json'))
  ...
  const promises = lernaPackages.map((pattern) => {
    return new Promise((resolve, reject) => {
      glob(pattern, { cwd: basePath }, (error, matches) => {
